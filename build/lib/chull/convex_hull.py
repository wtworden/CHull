...
class Polytope():
    ...
    def negative(self):
        if self.dim == 1:
            return Polytope(1,[Vertex(-self.vertices()[0].coords()),Vertex(-self.vertices()[1].coords())])
        else:
            return Polytope(self.dim,[f.negative for f in self.facets(self.dim-1)])
...
def quick_hull(vector_points, dim):
    ...
    # choose dim points for the initial hull. We might as well pick the ones of largest norm.
